...
@section('content')
    <div class="">
        <div class="my-3 px-2 md:px-4 text-end md:my-4">
            {{-- <a href="{{ route('ourCareer.create') }}"
            ...
            <div class="border border-[#000] max-h-[70vh] overflow-y-auto">
                <table class="min-w-full table-fixed">
                    ...
                    <tbody class="text-gray-700">
                        @foreach ($our_careers as $index => $our_career)
                            <tr class="border-t border-[#000]">
                                ...
                                <td class="text-left py-3 px-4 text-[10px] md:text-[12px] border-r border-[#000]">
                                    <div class="flex items-center h-full w-full">
                                        <img src="{{ asset($our_career->image) }}" alt="" class="w-20 h-auto">
                                    </div>
                                </td>
                                <td class="py-3 px-4 text-xs border-r border-[#000]">
                                    <div class="flex items-center h-full space-x-2 w-full">
                                        {{-- <a href="{{ route('ourCareer.delete', $our_career->id) }}" title="Delete"
                                            onclick="event.preventDefault(); deleteRecord('{{ route('ourCareer.delete', $our_career->id) }}')">
                                        ...
                                        <a href="{{ route('ourCareer.edit', $our_career->id) }}" title="Edit">
