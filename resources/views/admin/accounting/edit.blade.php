<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Edit OurService (Accounting & Bookkeeping)</h2>
        <form action="{{ route('accounting.update', $accounting->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <input value="{{ old('title.en', $accounting->title['en'] ?? '') }}" type="text"
                    ...
                    @if (!empty($accounting->content3['en']))
                        <div>
                            ...
                            <textarea name="content3[en]" id="content3_en" rows="4"
                                class="mt-1 block w-full p-2 border rounded-md text-[#000] text-sm">{{ old('content3.en', $accounting->content3['en'] ?? '') }}</textarea>
                    ...
                    @if (!empty($accounting->content3['ch']))
                        <div>
                            ...
                            <textarea name="content3[ch]" id="content3_ch" rows="4"
                                class="mt-1 block w-full p-2 border rounded-md text-[#000] text-sm">{{ old('content3.ch', $accounting->content3['ch'] ?? '') }}</textarea>
            ...
            <!-- Dropzone for Image -->
            @if (!empty($accounting->image))
                <div>
                    <label for="dropzone-file{{ $accounting->id }}" id="drop-area"
                        ...
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                            id="img-preview" style="background-image: url({{ asset($accounting->image) }})">
                        ...
                        <input id="dropzone-file{{ $accounting->id }}" type="file" class="hidden" name="image"
