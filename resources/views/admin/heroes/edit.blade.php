    ...
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        ...
        <form action="{{ route('hero.update', $hero->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <h2 class="text-[20px] font-[600] text-[#DFAD16]">English</h2>
                    <label for="dropzone-file-en{{ $hero->id }}" id="drop-area-en"
                        class="flex flex-col items-center justify-center w-full h-64 border-2 mt-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                            id="img-preview-en" style="background-image: url({{ asset($hero->images['en'] ?? '') }})">
                        ...
                        <input id="dropzone-file-en{{ $hero->id }}" type="file" class="hidden" name="images[en]" accept="image/*"
                ...
                <div>
                    <h2 class="text-[20px] font-[600] text-[#DFAD16]">Chinese</h2>
                    <label for="dropzone-file-ch{{ $hero->id }}" id="drop-area-ch"
                        class="flex flex-col items-center justify-center w-full h-64 border-2 mt-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                            id="img-preview-ch" style="background-image: url({{ asset($hero->images['ch'] ?? '') }})">
                            <svg class="w-8 h-8 mb-4 text-[#000]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                fill="none" viewBox="0 0 20 16">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="1"
                        ...
                        <input id="dropzone-file-ch{{ $hero->id }}" type="file" class="hidden" name="images[ch]" accept="image/*"
