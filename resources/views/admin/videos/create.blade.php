<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Add Video</h2>
        <form action="{{ route('profile-video.store') }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="dropzone-file-en" id="drop-area-en"
                        ...
                        <input id="dropzone-file-en" type="file" class="hidden" name="videos_en" accept="video/*"
                            data-lang="en" onchange="uploadVideo(event, 'img-preview-en')" />
                    </label>
                    <x-input-error class="mt-2" :messages="$errors->get('videos_en')" />
                </div>
                ...
                <div>
                    <label for="dropzone-file-ch" id="drop-area-ch"
                        ...
                        <input id="dropzone-file-ch" type="file" class="hidden" name="videos_ch" accept="video/*"
                            onchange="uploadVideo(event, 'img-preview-ch')"  />
                    </label>
                    <x-input-error class="mt-2" :messages="$errors->get('videos_ch')" data-lang="ch" />
                </div>
            </div>
