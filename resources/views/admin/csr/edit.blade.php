<x-app-layout>
    <div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        ...
        <form action="{{ route('csr.update', $csr->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <!-- Dropzone for Image -->
            <div>
                <label for="dropzone-file{{ $csr->id }}" id="drop-area"
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                        id="img-preview" style="background-image: url({{ asset($csr->image) }})">
                    ...
                    <input id="dropzone-file{{ $csr->id }}" type="file" class="hidden" name="image" accept="image/*"
