<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        ...
        <form action="{{ route('homepage.update', $home->id) }}" method="POST" enctype="multipart/form-data"
            ...
            @endcomponent
            @if ($home->id >= 1 && $home->id <= 5)
                <div class="flex flex-col items-end justify-start">
                    <label for="active" class="block text-sm font-medium text-gray-700">Active</label>
                    <input type="checkbox" name="active" id="active" value="1"
                        {{ $home->active == 1 ? 'checked' : '' }}
                        class="mt-1 p-2 border rounded-md focus:ring-[#DFAD16] focus:border-[#DFAD16] text-black text-sm">
                ...
                <div class="p-0 space-y-4">
                    ...
                    @if (!empty($home->title['en']))
                        <div>
                            ...
                            <input value="{{ old('title.en', $home->title['en'] ?? '') }}" type="text" name="title[en]"
                                id="title_en"
                    ...
                    @if (!empty($home->content['en']))
                        <div>
                            ...
                            <textarea name="content[en]" id="content_en" rows="6"
                                class="mt-1 block w-full p-2 border rounded-md focus:ring-[#DFAD16] focus:border-[#DFAD16] text-black text-[12px]">{{ old('content.en', $home->content['en'] ?? '') }}</textarea>
                    ...
                    @if (!empty($home->content['ch']))
                        <div>
                            ...
                            <textarea name="content[ch]" id="content_ch" rows="6"
                                class="mt-1 block w-full p-2 border rounded-md focus:ring-[#DFAD16] focus:border-[#DFAD16] text-black text-[12px]">{{ old('content.ch', $home->content['ch'] ?? '') }}</textarea>
            ...
            @if (!empty($home->icon))
                <div>
                    <label for="dropzone-file{{ $home->id }}" id="drop-area"
                        ...
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                            id="img-preview" style="background-image: url({{ asset($home->icon) }})">
                        ...
                        <input id="dropzone-file{{ $home->id }}" type="file" class="hidden" name="icon"
