<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Edit Uniqueness</h2>
        <form action="{{ route('uniqueness.update', $uniq->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <input value="{{ old('title.en', $uniq->title['en'] ?? '') }}" type="text" name="title[en]"
                            id="title_en"
                    ...
                    <div>
                        ...
                        <textarea name="content[en]" id="content_en" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md focus:ring-[#DFAD16] focus:border-[#DFAD16] text-black text-[12px]">{{ old('content.en', $uniq->content['en'] ?? '') }}</textarea>
                ...
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <input value="{{ old('title.ch', $uniq->title['ch'] ?? '') }}" type="text" name="title[ch]"
                            id="title_ch"
                            class="mt-1 block w-full p-2 border rounded-md focus:ring-[#DFAD16] focus:border-[#DFAD16] text-black text-sm">
                    ...
                    <div>
                        ...
                        <textarea name="content[ch]" id="content_ch" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md focus:ring-[#DFAD16] focus:border-[#DFAD16] text-black text-[12px]">{{ old('content.ch', $uniq->content['ch'] ?? '') }}</textarea>
