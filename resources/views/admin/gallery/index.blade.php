...
@section('content')
    <div class="" x-data="reorderTable()" x-init="initSortable()">
        <div class="my-2 px-2 md:px-4 flex items-center justify-between">
            <a href="{{ route('galleryImage.create') }}"
                ...
                <table class="min-w-full table-fixed">
                    ...
                    <tbody x-ref="tableBody" class="text-gray-700">
                        @foreach ($galleries as $index => $gallery)
                            <tr class="border-t border-[#000] cursor-move" draggable="true" x-bind:data-id="{{ $gallery->id }}"
                                @dragstart="dragStart($event, {{ $gallery->id }})" @dragover.prevent
                                @drop="drop($event, {{ $gallery->id }})">
                                {{-- <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#000]">
                                    <div class="flex items-center h-full w-full">{{ $index + 1 }}</div>
                                </td> --}}
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#000]">
                                    <div class="flex items-center h-full w-full">{{ $gallery->order }}</div>
                                </td>
                                <td class="text-left py-3 px-4 text-[10px] md:text-[12px] border-r border-[#000]">
                                    <div class="flex items-center h-full w-full">
                                        <img src="{{ asset($gallery->image) }}" alt="" class="w-20 h-20 object-cover object-center">
                                    </div>
                                </td>
                                <td class="py-3 px-4 text-xs border-r border-[#000]">
                                    <div class="flex items-center h-full space-x-2 w-full">
                                        <a href="{{ route('galleryImage.delete', $gallery->id) }}" title="Delete"
                                            onclick="event.preventDefault(); deleteRecord('{{ route('galleryImage.delete', $gallery->id) }}')">
                                        ...
                                        <a href="{{ route('galleryImage.edit', $gallery->id) }}" title="Edit">
                    ...
                    this.$nextTick(() => {
                        ...
                        new Sortable(tableBody, {
                            ...
                            onEnd: async (event) => {
                                ...
                                let response = await fetch("{{ route('galleryImage.reorder') }}", {
                                    method: "POST",
                                    headers: {
