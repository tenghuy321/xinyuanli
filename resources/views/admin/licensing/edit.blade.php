<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Edit OurService</h2>
        <form action="{{ route('licensing.update', $licensing->id) }}" method="POST"
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    @if (!empty($licensing->content['en']))
                        <div>
                            ...
                            <textarea name="content[en]" id="content_en" rows="4"
                                class="mt-1 block w-full p-2 border rounded-md text-[#000] text-sm">{{ old('content.en', $licensing->content['en'] ?? '') }}</textarea>
                ...
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <input value="{{ old('title.ch', $licensing->title['ch'] ?? '') }}" type="text" name="title[ch]" id="title_ch"
                            class="mt-1 block w-full p-2 border rounded-md text-[#000] text-sm">
                    ...
                    @if (!empty($licensing->content['en']))
                        <div>
                            ...
                            <textarea name="content[ch]" id="content_ch" rows="4"
                                class="mt-1 block w-full p-2 border rounded-md text-[#000] text-sm">{{ old('content.ch', $licensing->content['ch'] ?? '') }}</textarea>
            ...
            @if (!empty($licensing->image))
                <div>
                    <label for="dropzone-file{{ $licensing->id }}" id="drop-area"
                        ...
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                            id="img-preview" style="background-image: url({{ asset($licensing->image) }})">
                        ...
                        <input id="dropzone-file{{ $licensing->id }}" type="file" class="hidden" name="image" accept="image/*"
